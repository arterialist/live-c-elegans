...
/** Live controls for every ``mjModel.opt`` field exposed in ``/api/schema``. */
export function MujocoEngineTab() {
  ...
  const groups = useMemo(() => groupMujoco(specs), [specs]);
  ...
  return (
    <TabShell
      title="MuJoCo engine"
      subtitle="All mjOption fields: timestep, integrator, solver, fluid, gravity, global contact overrides, and engine flags. Live — Submit applies on the next tick."
    >
...
function groupMujoco(specs: ParameterSpec[]): [string, ParameterSpec[]][] {
